...
export function ImageEditor({ url, onClose, onApplyResult }: ImageEditorProps) {
  ...
  const draw = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (!isDrawing || activeTab !== 'brush' || !maskCanvasRef.current) return
    ...
    if (isEraser) {
      ...
      ctx.strokeStyle = 'rgba(124, 58, 237, 0.5)' // Primary color with alpha
      maskCtx.strokeStyle = '#ffffff' // White for mask
    }
    ...
    if (channelRef.current) {
      channelRef.current.publish('brush', { 
        x: scaledX, 
        y: scaledY, 
  ...
  const handleRemoteDraw = (data: any) => {
    const { x, y, size, isEraser, isStart } = data
    const canvas = canvasRef.current
    const maskCanvas = maskCanvasRef.current
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="w-full max-w-[1400px] h-[92vh] max-h-[900px] bg-card/95 backdrop-blur-2xl rounded-[2rem] border border-border/30 shadow-2xl shadow-black/50 flex flex-col overflow-hidden"
      >
        ...
        <div className="flex-1 flex flex-row min-h-0 overflow-hidden">
          ...
          {/* ── ZONE 1: TOOL DOCK ── */}
          <div className="w-[72px] shrink-0 border-r border-border/20 flex flex-col items-center py-4 gap-1 bg-background/30 backdrop-blur-sm">
            {toolDockItems.map((tool) => (
              <div key={tool.id} className="relative w-full flex flex-col items-center">
                <button
                  onClick={() => setActiveTab(tool.id)}
                  className={cn(
                    "w-[52px] h-[52px] rounded-xl flex flex-col items-center justify-center gap-1 transition-all duration-200 active:scale-95",
                    activeTab === tool.id
          ...
          {/* ── ZONE 2: TOOL PANEL ── */}
          <div className="w-[300px] shrink-0 border-r border-border/20 flex flex-col bg-background/20 backdrop-blur-sm overflow-hidden">
          ...
          {/* ── ZONE 3: CANVAS ── */}
          <div className="flex-1 flex flex-col p-4 gap-3 bg-background/10 overflow-hidden min-w-0">
            ...
            >
                    ...
                    {activeTab === 'brush' && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <canvas
                          ...
                          width={1024}
                          height={1024}
                          className="w-auto h-auto max-w-full max-h-full object-contain pointer-events-auto cursor-crosshair"
                        />
              ...
              {/* Hidden mask canvas */}
              <canvas ref={maskCanvasRef} width={1024} height={1024} className="hidden" />
        ...
        {/* ══════════════════════════════════════════════════════════
            FOOTER BAR
        ══════════════════════════════════════════════════════════ */}
        <div className="h-[52px] flex items-center justify-between px-5 border-t border-border/20 bg-background/50 backdrop-blur-sm shrink-0 gap-4">
